...
function Composition(props) {
	...
	const [numRows, ] = React.useState(
		props.numRows
	)
	let maxNumRows = 24; // Has to be a multiple of props.numRows
	const [numColumns, ] = React.useState(
		props.numColumns
	)
	let maxNumColumns = 20; // Has to be a multiple of props.numColumns
	const [virtualGridLayout, ] = React.useState(
		props.layout.split(" ")
	)
	console.log(virtualGridLayout)
	// console.log(props.children)
	if (virtualGridLayout.length !== props.children.length)
	...
	function getCellCoordinatesFromIndex(index) {
		let cellLayout = virtualGridLayout[index];
			...
			updateGrid(
				Utils.objectToArray(
					deepmerge(
						...
						parseCellLayout(cellLayout).map((element, index) => {
	...
	let columnSizes = Utils.arraySplit(window.innerWidth, maxNumColumns)
	...
	let rowSizes = Utils.arraySplit(window.innerWidth, maxNumRows)
		...
		<div style={gridContainerStyle}>
			{
				props.children.map((element, index) => {
					 ...
					 */
					let vcc = toVirtualCellCoordinates(
